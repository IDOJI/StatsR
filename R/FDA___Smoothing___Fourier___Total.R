FDA___Smoothing___Fourier___Total = function(y = y, n_basis = nbasis, lambdas = lambdas, penalty.type = "harmonic", criterion = c("OCV", "GCV")){
  ...
  ### seq_2
  lambdas = exp(seq(log(opt_smoothing_1$lambda) - 10 , log(opt_smoothing_1$lambda) + 10, by = 0.1))
  ...
  ### seq_3
  lambdas = exp(seq(log(opt_smoothing_2$lambda) - 1 , log(opt_smoothing_2$lambda) + 1 , by = 0.01))
  ...
  ### seq_4
  lambdas = exp(seq(log(opt_smoothing_3$lambda) - 0.1 , log(opt_smoothing_3$lambda) + 0.1 , by = 0.001))
